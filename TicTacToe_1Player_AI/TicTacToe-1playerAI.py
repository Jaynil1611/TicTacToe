...
def computermove(move,p):
        print(move)
        game[move]=p
        print_game()
                
def check_result(game,p1,p2):
...
def isFree(game1,move):
    return game1[move]==' ';

def computer_attack(p1,p2):
    game1=[]
    for i in range(9):
        game1.clear()
        game1=game.copy()
...
def begin():
    ...
    while True:
        ...
        player(player1)
        print(count)
        n=check_result(game,player1,player2)
        if n==1:
            sys.exit()
        if(count>=9):
            print("Game Drawn ")
...
#        computer_attack is the function with attacking & defensing ability 
        m=computer_attack(player1,player2)
        count+=1
        if(m!=-1):
#            computermove is for actually playing the turn in game
            computermove(m,player2)
        else:
